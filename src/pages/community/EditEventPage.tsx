...
const EditEventPage = () => {
  ...
  const { user } = useAuth();
  const { updateEvent } = useCommunity();
  ...
  useEffect(() => {
    if (event) {
      ...
      if (event.user_id !== user?.id) {
        toast({ title: "Unauthorized", description: "You can only edit your own events", variant: "destructive" });
        navigate("/community");
        return;
      }

      const locationValue = event.is_online 
        ? (event.venue_name ? "hybrid" : "virtual") 
        : "in-person";
      ...
      setFormData({
        ...
        duration: String(event.duration_minutes || 60),
    ...
    try {
      await updateEvent.mutateAsync({
        ...
        data: {
          ...
          location: formData.location !== "virtual" ? formData.venueName.trim() || null : null,
          cover_image: formData.coverImage || null,
          venue_name: formData.venueName.trim() || null,
          full_address: formData.fullAddress.trim() || null,
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12 max-w-4xl">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="text-xl sm:text-2xl">Edit Event</CardTitle>
            <CardDescription className="text-sm sm:text-base">
              Update your event details. Registered attendees will be notified of changes.
            </CardDescription>
          </CardHeader>
          <CardContent>
